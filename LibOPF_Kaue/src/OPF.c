...
//Classification function: it classifies samples from sg and marks the relevant nodes-----
void opf_OPFClassifyingMarkingRelevants(Subgraph *sgtrain, Subgraph *sg){

    int i, j, k, l, label = -1;
    float tmp, weight, minCost, *cost = NULL;

    cost = AllocFloatArray(sgtrain->nnodes);
    ...
    for (i = 0; i < sg->nnodes; i++){
        ...
        for (j = 0; j < sgtrain->nnodes; j++){
            if((cost[j] == minCost) && (sgtrain->node[j].label == label)){
                if(!sgtrain->node[j].relevant){
                    sgtrain->node[j].relevant = 1;
                    opf_MarkNodes(sgtrain,j);
                }
                j = sgtrain->nnodes;
            }
        }
...
void opf_OPFPrototypesClassify(){}

// Classification function
void opf_OPFKMeansClassify(){}

// Classification function
void opf_OPFRandClassify(){}
...
void opf_OPFPruning(Subgraph **sgtrain, Subgraph **sgeval, float dAcc){
    ...
    opf_OPFClassifying(*sgtrain, *sgeval);
    Acc = opf_Accuracy(*sgeval);
    sg = CopySubgraph(*sgtrain); tAcc = Acc;
    //fflush(stdout); fprintf(stdout,"\nInitial accuracy in the evaluation set: %.2f %%\n", Acc*100);
    while((fabs(Acc-tAcc) <= dAcc) && (i <= iterations)){
        //fflush(stdout); fprintf(stdout, "\nrunning iteration ... %d of pruning *****", i);
        opf_OPFLearning(&(*sgtrain), &(*sgeval));
        opf_OPFClassifyingMarkingRelevants(*sgtrain, *sgeval);
        opf_MoveIrrelevantNodes(&(*sgtrain), &(*sgeval));
        if((*sgtrain)->nnodes){
            opf_OPFTraining(*sgtrain);
            opf_OPFClassifying(*sgtrain, *sgeval);
            tAcc = opf_Accuracy(*sgeval);
            if (fabs(Acc-tAcc) <= dAcc){
                if (sg!=NULL) DestroySubgraph(&sg);
                sg = CopySubgraph(*sgtrain);
            }

            //fflush(stdout); fprintf(stdout,"Accuracy in the evaluation set: %.2f %%\n", tAcc*100);
            //fprintf(stderr,"\nAcc-tAcc: %f	dAcc: %f",fabs(Acc-tAcc), dAcc);
            i++;
        }else i = iterations+1;
    }
    DestroySubgraph(&(*sgtrain));
